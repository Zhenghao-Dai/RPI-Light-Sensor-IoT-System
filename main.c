...
#define SENSE_INTERVAL_SEC  (2000000)
...
int main(void)
{
    ...
    //create and assign udp
    mutex_init(&msg_lock);
    udp_rx_args_t* temp = malloc(sizeof(udp_rx_args_t));
    temp->udp_port = UDP_RX_PORT;
    // temp.udp_send_port = SEND_PORT
    
    temp->mutex = &msg_lock;
    printf("readched here");
    udp_rx_init(temp);
    ...
    while(1)
    {
        ...
        //format payload
        char buffer [21];
        sprintf(buffer, "zhenghad/light: %d", sample);
        //send udp pkt
        mutex_lock(&msg_lock);
        udp_send(RELAY_NODE_ADDR, UDP_SEND_PORT_STR, buffer); // (char *addr_str, char *port_str, char *data)
        //unlock here
        mutex_unlock(&msg_lock);
